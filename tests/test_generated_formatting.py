...
def test_generated_files_pass_ruff_and_are_indented(tmp_path: Path, orm: str, async_mode: bool) -> None:
    cfg_text = "\n".join(
        [
            f"orm: {orm}",
    ...
    for fpath in written:
        ...
        res_check = subprocess.run(["ruff", "check", str(fpath)], capture_output=True, text=True)
        ...
        for i, line in enumerate(lines[:-2]):
            if line.startswith("    def ") and i > 0:
                ...
                assert prev.strip() == "", f"missing blank line before method at line {i+1} in {fpath}"
